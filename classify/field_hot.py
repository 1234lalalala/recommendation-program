...
top_n = 20
file = '../classify/field'
class_num = {i.strip().split('\t')[0]:int(i.strip().split('\t')[1])     for i in open('../knowledge_graph/class_num.txt',encoding='utf8')}
...
def field_course(field):
    l = field_class[field]
    l.sort(key=lambda x:class_num[x] if x in class_num else 0,reverse=True)
    # print(l)
    return l[:top_n]

n = m = 0
length = 0
f =  open(f'../result/field_hot_{top_n}','w',encoding='utf8')
