...
file = '../classify/school'
id_school = {}
for each_file in os.listdir(file):
    with open(file+'/' +each_file,'r',encoding='utf8') as f:
        for line in f:
            id_school[line.strip().split('\t')[0]] = each_file[:-4]
def show_error_course(l,target):
    ...
    for i in l:
        if target[0] in relation[i] and relation[i][target[0]] > 0.1:
            print(course_all[i], course_all[target[0]],relation[i][target[0]])
        if i in relation[target[0]] and relation[target[0]][i] > 0.1:
            print(course_all[target[0]],course_all[i], relation[target[0]][i])
            # print(target[0] in dit[i])
...
def get_100(l):
    ...
    for i in l:
        if i in ret:
            ...
            ret[i] = 1
    l = [i for i in ret]
    l.sort(key = lambda x:ret[i],reverse=True)
    return l[:100]

dit_list = [dit]
f =  open(f'../result/field_result','w',encoding='utf8')
